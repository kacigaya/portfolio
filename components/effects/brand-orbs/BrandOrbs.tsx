...
import { BRAND_ORBS_ENGINE } from "./sources/engine.generated";
import { GAYA_MARK_PATH, GAYA_MARK_VIEWBOX } from "./gaya-mark";
...
export const BRAND_ORBS_DEFAULTS = {
  variant: "gaya" as BrandOrbVariant,
...
// The engine keeps MARK_PATHS and MODES private to its IIFE, so a new brand has
// to be spliced into the source text before it is handed to the iframe. Both
// anchors are asserted: a source bump that moves them fails loudly here rather
// than rendering a blank canvas.
function registerGayaMark(engine: string) {
  const marksAnchor = "  const MARK_PATHS = {\n    github:";
  const modesAnchor = "  const MODES = {\n    claude:";
  if (!engine.includes(marksAnchor) || !engine.includes(modesAnchor)) {
    throw new Error("brand-orbs-v2.html changed shape; re-check the gaya mark injection anchors");
  }
  const markEntry = `    gaya: ${JSON.stringify(GAYA_MARK_PATH)},\n`;
  // Same knobs the authored linkedin/css marks use: a slow vertical scan reads
  // best on a wide mark, and no accent keeps the orb monochrome like the SVG.
  // nMini runs hotter than the stock 12-13 because this mark carries two glasses
  // lenses; below 18 rows they close up and the orb reads as a plain blob.
  const modeEntry =
    `    gaya: { draw: mk({ key: "gaya", n: 30, nMini: 18, motion: "scan", speed: .34,` +
    ` vb: ${GAYA_MARK_VIEWBOX}, fit: .88, v: .64 }), accent: null, speed: 1, staticT: 1.2 },\n`;
  return engine
    .replace(marksAnchor, `  const MARK_PATHS = {\n${markEntry}    github:`)
    .replace(modesAnchor, `  const MODES = {\n${modeEntry}    claude:`);
}

const ORB_ENGINE = registerGayaMark(BRAND_ORBS_ENGINE).replace(/<\/script/gi, "<\\/script");
